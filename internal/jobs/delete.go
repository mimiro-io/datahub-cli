...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		pterm.DefaultSection.Println("Deleting job " + server + "/jobs/" + idOrTitle)

		id := ResolveId(server,token, idOrTitle)
		if confirm {
			pterm.DefaultSection.Printf("Delete job with job id " + idOrTitle + " on " + server + ", please type (y)es or (n)o and then press enter:")
